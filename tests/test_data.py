...
def test_sentence_to_real_string(tasks_base_path):
    ...
    assert ('Schartau sagte dem " Tagesspiegel " vom Freitag , Fischer sei " in einer Weise aufgetreten , die alles andere als überzeugend war " .' ==
...
def test_tagged_corpus_get_all_sentences():
    ...
    test_sentence = Sentence(
        'I will be only used for testing.', use_tokenizer=True)
...
def test_tagged_corpus_make_vocab_dictionary():
    train_sentence = Sentence(
        'used in training. training is cool.', use_tokenizer=True)
...
def test_tagged_corpus_statistics():
    train_sentence = Sentence('I love Berlin.', labels=[
                              Label('class_1')], use_tokenizer=True)
    dev_sentence = Sentence('The sun is shining.', labels=[
                            Label('class_2')], use_tokenizer=True)
    test_sentence = Sentence('Berlin is sunny.', labels=[
                             Label('class_1')], use_tokenizer=True)
...
def test_tagged_corpus_statistics_string_label():
    train_sentence = Sentence('I love Berlin.', labels=[
                              'class_1'], use_tokenizer=True)
    dev_sentence = Sentence('The sun is shining.', labels=[
                            'class_2'], use_tokenizer=True)
    test_sentence = Sentence('Berlin is sunny.', labels=[
                             'class_1'], use_tokenizer=True)
    class_to_count_dict = Corpus._get_class_to_count(
        [train_sentence, dev_sentence, test_sentence])
...
def test_tagged_corpus_statistics_multi_label():
    ...
    dev_sentence = Sentence('The sun is shining.', labels=[
                            'class_2'], use_tokenizer=True)
    test_sentence = Sentence('Berlin is sunny.', labels=[
                             'class_1', 'class_2'], use_tokenizer=True)
    class_to_count_dict = Corpus._get_class_to_count(
        [train_sentence, dev_sentence, test_sentence])
    assert ('class_1' in class_to_count_dict)
    assert ('class_2' in class_to_count_dict)
...
def test_tagged_corpus_downsample():
    sentence = Sentence('I love Berlin.', labels=[
                        Label('class_1')], use_tokenizer=True)
    corpus = Corpus([sentence, sentence, sentence, sentence, sentence,
...
def test_spans():
    ...
    spans = sentence.get_spans('ner', min_score=0.0)
    assert (2 == len(spans))
    assert ('Zalando Research' == spans[0].text)
    assert ('ORG' == spans[0].tag)
    ...
    spans = sentence.get_spans('ner', min_score=0.6)
    assert (1 == len(spans))
    spans = sentence.get_spans('ner', min_score=0.99)
    assert (0 == len(spans))
...
def test_sentence_to_dict():
    sentence = Sentence('Zalando Research is   located in Berlin, the capital of Germany.', labels=[
                        'business'], use_tokenizer=True)
    sentence[0].add_tag('ner', 'B-ORG')
    sentence[1].add_tag('ner', 'E-ORG')
    sentence[5].add_tag('ner', 'S-LOC')
    ...
    assert (
        'Zalando Research is   located in Berlin, the capital of Germany.' == dict['text'])
    ...
    dict = sentence.to_dict('ner')
    assert (
        'Facebook, Inc. is a company, and Google is one as well.' == dict['text'])
