...
from flair.embeddings import RoBERTaEmbeddings, OpenAIGPTEmbeddings, OpenAIGPT2Embeddings, XLNetEmbeddings, TransformerXLEmbeddings, XLMEmbeddings
from pytorch_transformers import RobertaModel, RobertaTokenizer, OpenAIGPTModel, OpenAIGPTTokenizer, GPT2Model, GPT2Tokenizer, XLNetModel, XLNetTokenizer, TransfoXLModel, TransfoXLTokenizer, XLMModel, XLMTokenizer
from typing import List


def calculate_mean_embedding(subword_embeddings: List[torch.FloatTensor]) -> torch.FloatTensor:
    all_embeddings = [embedding.unsqueeze(0)
                      for embedding in subword_embeddings]
...
def test_roberta_embeddings():
    ...
    def embed_sentence(sentence: str, pooling_operation, layers: str = '1', use_scalar_mix: bool = False) -> Sentence:
        embeddings = RoBERTaEmbeddings(pretrained_model_name_or_path=roberta_model,
                                       layers=layers, pooling_operation=pooling_operation, use_scalar_mix=use_scalar_mix)
        flair_sentence = Sentence(sentence)
        embeddings.embed(flair_sentence)
        return flair_sentence
    sentence_first_subword = embed_sentence(
        sentence=s, pooling_operation='first')
    ...
    puppeteer_first_subword_embedding_actual = sentence_first_subword.tokens[7].embedding.tolist(
    )
    ...
    sentence_last_subword = embed_sentence(
        sentence=s, pooling_operation='last')
    ...
    puppeteer_last_subword_embedding_actual = sentence_last_subword.tokens[7].embedding.tolist(
    )
    assert (first_token_embedding_ref == first_token_embedding_actual)
    ...
    first_token_embedding_ref = torch.cat(
        [first_layer[1], first_layer[2]]).tolist()
    first_token_embedding_actual = sentence_first_last_subword.tokens[0].embedding.tolist(
    )
    puppeteer_first_last_subword_embedding_ref = torch.cat(
        [first_layer[9], first_layer[11]]).tolist()
    puppeteer_first_last_subword_embedding_actual = sentence_first_last_subword.tokens[7].embedding.tolist(
    )
    assert (first_token_embedding_ref == first_token_embedding_actual)
    assert (puppeteer_first_last_subword_embedding_ref ==
            puppeteer_first_last_subword_embedding_actual)
    sentence_mean_subword = embed_sentence(
        sentence=s, pooling_operation='mean')
    ...
    puppeteer_mean_subword_embedding_actual = sentence_mean_subword.tokens[7].embedding.tolist(
    )
    assert (first_token_embedding_ref == first_token_embedding_actual)
    ...
    sentence_mult_layers_scalar_mix = embed_sentence(
        sentence='Berlin', pooling_operation='first', layers='1,2,3,4', use_scalar_mix=True)
...
def test_gpt_embeddings():
    ...
    def embed_sentence(sentence: str, pooling_operation, layers: str = '1', use_scalar_mix: bool = False) -> Sentence:
        embeddings = OpenAIGPTEmbeddings(pretrained_model_name_or_path=gpt_model, layers=layers,
    ...
    sentence_first_last_subword = embed_sentence(
        sentence=s, pooling_operation='first_last')
    first_token_embedding_ref = torch.cat(
        [first_layer[0], first_layer[0]]).tolist()
    first_token_embedding_actual = sentence_first_last_subword.tokens[0].embedding.tolist(
    )
    puppeteer_first_last_subword_embedding_ref = torch.cat(
...
def test_gpt2_embeddings():
    ...
    def embed_sentence(sentence: str, pooling_operation, layers: str = '1', use_scalar_mix: bool = False) -> Sentence:
        embeddings = OpenAIGPT2Embeddings(pretrained_model_name_or_path=gpt_model, layers=layers,
                                          pooling_operation=pooling_operation, use_scalar_mix=use_scalar_mix)
        flair_sentence = Sentence(sentence)
        embeddings.embed(flair_sentence)
        return flair_sentence
...
def test_xlnet_embeddings():
    ...
    def embed_sentence(sentence: str, pooling_operation, layers: str = '1', use_scalar_mix: bool = False) -> Sentence:
        embeddings = XLNetEmbeddings(pretrained_model_name_or_path=xlnet_model, layers=layers,
                                     pooling_operation=pooling_operation, use_scalar_mix=use_scalar_mix)
        flair_sentence = Sentence(sentence)
        embeddings.embed(flair_sentence)
        return flair_sentence
    ...
    sentence_first_last_subword = embed_sentence(
        sentence=s, pooling_operation='first_last')
    first_token_embedding_ref = torch.cat(
        [first_layer[1], first_layer[1]]).tolist()
    first_token_embedding_actual = sentence_first_last_subword.tokens[0].embedding.tolist(
    )
    puppeteer_first_last_subword_embedding_ref = torch.cat(
...
def test_transformer_xl_embeddings():
    ...
    def embed_sentence(sentence: str, layers: str = '1', use_scalar_mix: bool = False) -> Sentence:
...
def test_xlm_embeddings():
    ...
    def embed_sentence(sentence: str, pooling_operation, layers: str = '1', use_scalar_mix: bool = False) -> Sentence:
        embeddings = XLMEmbeddings(pretrained_model_name_or_path=xlm_model, layers=layers,
                                   pooling_operation=pooling_operation, use_scalar_mix=use_scalar_mix)
        flair_sentence = Sentence(sentence)
        embeddings.embed(flair_sentence)
        return flair_sentence
