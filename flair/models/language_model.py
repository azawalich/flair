...
class LanguageModel(nn.Module):
    ...
    def get_representation(self, strings: List[str], chars_per_chunk: int = 512):
        ...
        for chunk in chunks:
            ...
            batch = torch.tensor(
                sequences_as_char_indices, dtype=torch.long, device=flair.device).transpose(0, 1)
    ...
    def calculate_perplexity(self, text: str) -> float:
        if (not self.is_forward_lm):
            text = text[::(- 1)]
        input = torch.tensor([self.dictionary.get_idx_for_item(char)
                              for char in text[:(- 1)]]).unsqueeze(1)
        input = input.to(flair.device)
        hidden = self.init_hidden(1)
        (prediction, _, hidden) = self.forward(input, hidden)
        ...
        loss = cross_entroy_loss(prediction.view(
            (- 1), len(self.dictionary)), targets).item()
