...
from flair.training_utils import convert_labels_to_one_hot, Metric, Result, store_embeddings
...
class TextClassifier(flair.nn.Model):
    ...
    def __init__(self, document_embeddings: flair.embeddings.DocumentEmbeddings, label_dictionary: Dictionary, multi_label: bool = None, multi_label_threshold: float = 0.5):
    ...
    def _init_model_with_state_dict(state):
        model = TextClassifier(document_embeddings=state['document_embeddings'],
                               label_dictionary=state['label_dictionary'], multi_label=state['multi_label'])
    ...
    def predict(self, sentences: Union[(Sentence, List[Sentence])], mini_batch_size: int = 32, embedding_storage_mode='none', multi_class_prob: bool = False) -> List[Sentence]:
    ...
    def _fetch_model(model_name) -> str:
        ...
        model_map['de-offensive-language'] = '/'.join(
            [aws_resource_path, 'TEXT-CLASSIFICATION_germ-eval-2018_task-1', 'germ-eval-2018-task-1.pt'])
