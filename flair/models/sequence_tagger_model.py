...
class SequenceTagger(flair.nn.Model):
    ...
    def __init__(self, hidden_size: int, embeddings: TokenEmbeddings, tag_dictionary: Dictionary, tag_type: str, use_crf: bool = True, use_rnn: bool = True, rnn_layers: int = 1, dropout: float = 0.0, word_dropout: float = 0.05, locked_dropout: float = 0.5, train_initial_hidden_state: bool = False, pickle_module: str = 'pickle'):
        ...
        if self.use_rnn:
            ...
            if (self.rnn_type in ['LSTM', 'GRU']):
                self.rnn = getattr(torch.nn, self.rnn_type)(rnn_input_dim, hidden_size, num_layers=self.nlayers, dropout=(
                    0.0 if (self.nlayers == 1) else 0.5), bidirectional=True)
                if self.train_initial_hidden_state:
                    self.hs_initializer = torch.nn.init.xavier_normal_
                    self.lstm_init_h = Parameter(torch.randn(
                        (self.nlayers * num_directions), self.hidden_size), requires_grad=True)
                    self.lstm_init_c = Parameter(torch.randn(
                        (self.nlayers * num_directions), self.hidden_size), requires_grad=True)
    ...
    def _init_model_with_state_dict(state):
        use_dropout = (0.0 if (not ('use_dropout' in state.keys()))
                       else state['use_dropout'])
        use_word_dropout = (0.0 if (
            not ('use_word_dropout' in state.keys())) else state['use_word_dropout'])
        use_locked_dropout = (0.0 if (
            not ('use_locked_dropout' in state.keys())) else state['use_locked_dropout'])
        train_initial_hidden_state = (False if (not (
            'train_initial_hidden_state' in state.keys())) else state['train_initial_hidden_state'])
        model = SequenceTagger(hidden_size=state['hidden_size'], embeddings=state['embeddings'], tag_dictionary=state['tag_dictionary'], tag_type=state['tag_type'], use_crf=state['use_crf'],
                               use_rnn=state['use_rnn'], rnn_layers=state['rnn_layers'], dropout=use_dropout, word_dropout=use_word_dropout, locked_dropout=use_locked_dropout, train_initial_hidden_state=train_initial_hidden_state)
    ...
    def evaluate(self, data_loader: DataLoader, out_path: Path = None, embeddings_storage_mode: str = 'cpu') -> (Result, float):
        with torch.no_grad():
                ...
                for (sentence, sent_tags) in zip(batch, tags):
                    for (token, tag) in zip(sentence.tokens, sent_tags):
                        ...
                        eval_line = '{} {} {} {}\n'.format(token.text, token.get_tag(
                            self.tag_type).value, tag.value, tag.score)
            ...
            detailed_result = ''.join(['\nMICRO_AVG: acc ', '{}'.format(metric.micro_avg_accuracy()), ' - f1-score ', '{}'.format(
                metric.micro_avg_f_score()), '\nMACRO_AVG: acc ', '{}'.format(metric.macro_avg_accuracy()), ' - f1-score ', '{}'.format(metric.macro_avg_f_score())])
            for class_name in metric.get_classes():
                detailed_result += ''.join(['\n', '{:<10}'.format(class_name), ' tp: ', '{}'.format(metric.get_tp(class_name)), ' - fp: ', '{}'.format(metric.get_fp(class_name)), ' - fn: ', '{}'.format(metric.get_fn(class_name)), ' - tn: ', '{}'.format(metric.get_tn(
                    class_name)), ' - precision: ', '{:.4f}'.format(metric.precision(class_name)), ' - recall: ', '{:.4f}'.format(metric.recall(class_name)), ' - accuracy: ', '{:.4f}'.format(metric.accuracy(class_name)), ' - f1-score: ', '{:.4f}'.format(metric.f_score(class_name))])
            result = Result(main_score=metric.micro_avg_f_score(), log_line=''.join(['{}'.format(metric.precision()), '\t', '{}'.format(
                metric.recall()), '\t', '{}'.format(metric.micro_avg_f_score())]), log_header='PRECISION\tRECALL\tF1', detailed_results=detailed_result)
    ...
    def predict(self, sentences: Union[(List[Sentence], Sentence)], mini_batch_size=32, embedding_storage_mode='none', verbose=False) -> List[Sentence]:
        with torch.no_grad():
            ...
            for (i, batch) in enumerate(batches):
                if verbose:
                    batches.set_description(
                        ''.join(['Inferencing on batch ', '{}'.format(i)]))
                with torch.no_grad():
                ...
                for (sentence, sent_tags, sent_all_tags) in zip(batch, tags, all_tags):
                    for (token, tag, token_all_tags) in zip(sentence.tokens, sent_tags, sent_all_tags):
                        ...
                        token.add_tags_proba_dist(
                            self.tag_type, token_all_tags)
    ...
    def forward(self, sentences: List[Sentence]):
        ...
        if self.use_rnn:
            ...
            if self.train_initial_hidden_state:
                initial_hidden_state = [self.lstm_init_h.unsqueeze(1).repeat(1, len(
                    sentences), 1), self.lstm_init_c.unsqueeze(1).repeat(1, len(sentences), 1)]
    ...
    def _obtain_labels(self, feature, sentences) -> (List[List[Label]], List[List[List[Label]]]):
        ...
        for (feats, length) in zip(feature, lengths):
            ...
            all_tags.append([[Label(self.tag_dictionary.get_item_for_index(score_id), score) for (
                score_id, score) in enumerate(score_dist)] for score_dist in scores])
        ...
        for (index, (tag_id, tag_scores)) in enumerate(zip(best_path, scores)):
            if ((type(tag_id) != int) and (tag_id.item() != np.argmax(tag_scores))):
                ...
                (scores[index][tag_id.item()], scores[index][swap_index_score]) = (
                    scores[index][swap_index_score], scores[index][tag_id.item()])
            elif ((type(tag_id) == int) and (tag_id != np.argmax(tag_scores))):
                swap_index_score = np.argmax(tag_scores)
                (scores[index][tag_id], scores[index][swap_index_score]) = (
                    scores[index][swap_index_score], scores[index][tag_id])
    ...
    def _filter_empty_sentences(sentences: List[Sentence]) -> List[Sentence]:
        ...
        if (len(sentences) != len(filtered_sentences)):
            log.warning('Ignore {} sentence(s) with no tokens.'.format(
    ...
    def _fetch_model(model_name) -> str:
        ...
        model_map['ner'] = '/'.join([aws_resource_path_v04,
                                     'NER-conll03-english', 'en-ner-conll03-v0.4.pt'])
        ...
        model_map['ner-ontonotes'] = '/'.join(
            [aws_resource_path, 'NER-ontoner--h256-l1-b32-%2Bcrawl%2Bnews-forward%2Bnews-backward--v0.2', 'en-ner-ontonotes-v0.3.pt'])
        model_map['ner-ontonotes-fast'] = '/'.join(
            [aws_resource_path, 'NER-ontoner--h256-l1-b32-%2Bcrawl%2Bnews-forward-fast%2Bnews-backward-fast--v0.2', 'en-ner-ontonotes-fast-v0.3.pt'])
        for key in ['ner-multi', 'multi-ner']:
            model_map[key] = '/'.join([aws_resource_path_v04,
                                       'release-quadner-512-l2-multi-embed', 'quadner-large.pt'])
        for key in ['ner-multi-fast', 'multi-ner-fast']:
            model_map[key] = '/'.join([aws_resource_path_v04,
                                       'NER-multi-fast', 'ner-multi-fast.pt'])
        for key in ['ner-multi-fast-learn', 'multi-ner-fast-learn']:
            model_map[key] = '/'.join([aws_resource_path_v04,
                                       'NER-multi-fast-evolve', 'ner-multi-fast-learn.pt'])
        model_map['pos'] = '/'.join([aws_resource_path_v04,
                                     'POS-ontonotes--h256-l1-b32-p3-0.5-%2Bglove%2Bnews-forward%2Bnews-backward-normal-locked0.5-word0.05--v0.4_0', 'en-pos-ontonotes-v0.4.pt'])
        model_map['pos-fast'] = '/'.join(
            [aws_resource_path, 'POS-ontonotes--h256-l1-b32-%2Bnews-forward-fast%2Bnews-backward-fast--v0.2', 'en-pos-ontonotes-fast-v0.2.pt'])
        for key in ['pos-multi', 'multi-pos']:
            model_map[key] = '/'.join([aws_resource_path_v04,
                                       'release-dodekapos-512-l2-multi', 'pos-multi-v0.1.pt'])
        for key in ['pos-multi-fast', 'multi-pos-fast']:
            model_map[key] = '/'.join([aws_resource_path_v04,
                                       'UPOS-multi-fast', 'pos-multi-fast.pt'])
        model_map['frame'] = '/'.join([aws_resource_path,
                                       'FRAME-conll12--h256-l1-b8-%2Bnews%2Bnews-forward%2Bnews-backward--v0.2', 'en-frame-ontonotes-v0.2.pt'])
        model_map['frame-fast'] = '/'.join(
            [aws_resource_path, 'FRAME-conll12--h256-l1-b8-%2Bnews%2Bnews-forward-fast%2Bnews-backward-fast--v0.2', 'en-frame-ontonotes-fast-v0.2.pt'])
        model_map['chunk'] = '/'.join([aws_resource_path_v04,
                                       'NP-conll2000--h256-l1-b32-p3-0.5-%2Bnews-forward%2Bnews-backward-normal-locked0.5-word0.05--v0.4_0', 'en-chunk-conll2000-v0.4.pt'])
        model_map['chunk-fast'] = '/'.join(
            [aws_resource_path, 'NP-conll2000--h256-l1-b32-%2Bnews-forward-fast%2Bnews-backward-fast--v0.2', 'en-chunk-conll2000-fast-v0.2.pt'])
        model_map['de-pos'] = '/'.join(
            [aws_resource_path, 'UPOS-udgerman--h256-l1-b8-%2Bgerman-forward%2Bgerman-backward--v0.2', 'de-pos-ud-v0.2.pt'])
        model_map['de-pos-fine-grained'] = '/'.join(
            [aws_resource_path_v04, 'POS-fine-grained-german-tweets', 'de-pos-twitter-v0.1.pt'])
        model_map['de-ner'] = '/'.join(
            [aws_resource_path, 'NER-conll03ger--h256-l1-b32-%2Bde-fasttext%2Bgerman-forward%2Bgerman-backward--v0.2', 'de-ner-conll03-v0.3.pt'])
        model_map['de-ner-germeval'] = '/'.join(
            [aws_resource_path_v04, 'NER-germeval', 'de-ner-germeval-0.4.1.pt'])
        model_map['fr-ner'] = '/'.join([aws_resource_path,
                                        'NER-aij-wikiner-fr-wp3', 'fr-ner.pt'])
        model_map['nl-ner'] = '/'.join([aws_resource_path_v04,
                                        'NER-conll2002-dutch', 'nl-ner-conll02-v0.1.pt'])
    ...
    def get_transition_matrix(self):
        ...
        for (to_idx, row) in enumerate(self.transitions):
            for (from_idx, column) in enumerate(row):
                row = [self.tag_dictionary.get_item_for_index(
                    from_idx), self.tag_dictionary.get_item_for_index(to_idx), column.item()]
                data.append(row)
            data.append(['----'])
