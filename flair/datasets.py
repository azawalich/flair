...
class ColumnCorpus(Corpus):
    ...
    def __init__(self, data_folder, column_format, train_file=None, test_file=None, dev_file=None, tag_to_bioes=None, comment_symbol=None, in_memory=True):
        ...
        train = ColumnDataset(train_file, column_format, tag_to_bioes,
                              comment_symbol=comment_symbol, in_memory=in_memory)
        if (test_file is not None):
            test = ColumnDataset(test_file, column_format, tag_to_bioes,
                                 comment_symbol=comment_symbol, in_memory=in_memory)
        else:
            train_length = len(train)
            test_size = round((train_length / 10))
            splits = random_split(
                train, [(train_length - test_size), test_size])
            train = splits[0]
            test = splits[1]
        if (dev_file is not None):
            dev = ColumnDataset(dev_file, column_format, tag_to_bioes,
                                comment_symbol=comment_symbol, in_memory=in_memory)
        else:
            train_length = len(train)
            dev_size = round((train_length / 10))
...
class UniversalDependenciesCorpus(Corpus):
    ...
    def __init__(self, data_folder, train_file=None, test_file=None, dev_file=None, in_memory=True):
...
class ClassificationCorpus(Corpus):
    ...
    def __init__(self, data_folder, train_file=None, test_file=None, dev_file=None, use_tokenizer=True, max_tokens_per_doc=(- 1), max_chars_per_doc=(- 1), in_memory=False):
        ...
        log.info('Dev: {}'.format(dev_file))
        log.info('Test: {}'.format(test_file))
        train = ClassificationDataset(train_file, use_tokenizer=use_tokenizer,
                                      max_tokens_per_doc=max_tokens_per_doc, max_chars_per_doc=max_chars_per_doc, in_memory=in_memory)
        test = ClassificationDataset(test_file, use_tokenizer=use_tokenizer,
                                     max_tokens_per_doc=max_tokens_per_doc, max_chars_per_doc=max_chars_per_doc, in_memory=in_memory)
        if (dev_file is not None):
            dev = ClassificationDataset(dev_file, use_tokenizer=use_tokenizer, max_tokens_per_doc=max_tokens_per_doc,
                                        max_chars_per_doc=max_chars_per_doc, in_memory=in_memory)
        else:
            train_length = len(train)
            dev_size = round((train_length / 10))
            splits = random_split(train, [(train_length - dev_size), dev_size])
...
class CSVClassificationCorpus(Corpus):
    ...
    def __init__(self, data_folder, column_name_map, train_file=None, test_file=None, dev_file=None, use_tokenizer=True, max_tokens_per_doc=(- 1), max_chars_per_doc=(- 1), in_memory=False, skip_header=False, **fmtparams):
        ...
        log.info('Dev: {}'.format(dev_file))
        log.info('Test: {}'.format(test_file))
        train = CSVClassificationDataset(train_file, column_name_map, use_tokenizer=use_tokenizer, max_tokens_per_doc=max_tokens_per_doc,
                                         max_chars_per_doc=max_chars_per_doc, in_memory=in_memory, skip_header=skip_header, **fmtparams)
        if (test_file is not None):
            test = CSVClassificationDataset(test_file, column_name_map, use_tokenizer=use_tokenizer, max_tokens_per_doc=max_tokens_per_doc,
                                            max_chars_per_doc=max_chars_per_doc, in_memory=in_memory, skip_header=skip_header, **fmtparams)
        else:
            train_length = len(train)
            test_size = round((train_length / 10))
            ...
            test = splits[1]
        if (dev_file is not None):
            dev = CSVClassificationDataset(dev_file, column_name_map, use_tokenizer=use_tokenizer, max_tokens_per_doc=max_tokens_per_doc,
                                           max_chars_per_doc=max_chars_per_doc, in_memory=in_memory, skip_header=skip_header, **fmtparams)
        else:
            train_length = len(train)
            dev_size = round((train_length / 10))
...
class ColumnDataset(FlairDataset):
    ...
    def __init__(self, path_to_column_file, column_name_map, tag_to_bioes=None, comment_symbol=None, in_memory=True):
        ...
        try:
            lines = open(str(path_to_column_file),
                         encoding='utf-8').read(10).strip().split('\n')
        except:
            log.info(
...
class UniversalDependenciesDataset(FlairDataset):
    ...
    def __init__(self, path_to_conll_file, in_memory=True):
...
class CSVClassificationDataset(FlairDataset):
    ...
    def __init__(self, path_to_file, column_name_map, max_tokens_per_doc=(- 1), max_chars_per_doc=(- 1), use_tokenizer=True, in_memory=True, skip_header=False, **fmtparams):
...
class ClassificationDataset(FlairDataset):
    ...
    def __init__(self, path_to_file, max_tokens_per_doc=(- 1), max_chars_per_doc=(- 1), use_tokenizer=True, in_memory=True):
    ...
    def _parse_line_to_sentence(self, line, label_prefix, use_tokenizer=True):
        words = line.split()
        labels = []
        l_len = 0
        ...
        if (text and labels):
            ...
            if ((sentence is not None) and (len(sentence) > self.max_tokens_per_doc) and (self.max_tokens_per_doc > 0)):
                sentence.tokens = sentence.tokens[:self.max_tokens_per_doc]
            return sentence
        return None
...
class CONLL_03(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=True):
        columns = {
            0: 'text',
            1: 'pos',
            2: 'np',
        ...
        if (not data_folder.exists()):
            log.warning(('-' * 100))
            log.warning(''.join(
                ['ACHTUNG: CoNLL-03 dataset not found at "', '{}'.format(data_folder), '".']))
            log.warning(
                'Instructions for obtaining the data can be found here: https://www.clips.uantwerpen.be/conll2003/ner/"')
            log.warning(('-' * 100))
        super(CONLL_03, self).__init__(data_folder, columns,
                                       tag_to_bioes=tag_to_bioes, in_memory=in_memory)
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=True):
        ...
        if (not data_folder.exists()):
            ...
            log.warning(
                'Instructions for obtaining the data can be found here: https://www.clips.uantwerpen.be/conll2003/ner/"')
            log.warning(('-' * 100))
        super(CONLL_03_GERMAN, self).__init__(data_folder, columns,
                                              tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class CONLL_03_DUTCH(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=True):
        ...
        cached_path(''.join(['{}'.format(conll_02_path),
                             'ned.testa']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(conll_02_path),
                             'ned.testb']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(conll_02_path),
                             'ned.train']), (Path('datasets') / dataset_name))
        super(CONLL_03_DUTCH, self).__init__(data_folder, columns,
                                             tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class CONLL_03_SPANISH(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=True):
        ...
        conll_02_path = 'https://www.clips.uantwerpen.be/conll2002/ner/data/'
        cached_path(''.join(['{}'.format(conll_02_path),
                             'esp.testa']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(conll_02_path),
                             'esp.testb']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(conll_02_path),
                             'esp.train']), (Path('datasets') / dataset_name))
        super(CONLL_03_SPANISH, self).__init__(data_folder, columns,
                                               tag_to_bioes=tag_to_bioes, in_memory=in_memory)


class CONLL_2000(ColumnCorpus):

    def __init__(self, base_path=None, tag_to_bioes='np', in_memory=True):
        columns = {
            0: 'text',
            1: 'pos',
            2: 'np',
        ...
        data_file = (((Path(flair.cache_root) / 'datasets') /
        ...
        if (not data_file.is_file()):
            ...
            import gzip
            import shutil
            with gzip.open((((Path(flair.cache_root) / 'datasets') / dataset_name) / 'train.txt.gz'), 'rb') as f_in:
                with open((((Path(flair.cache_root) / 'datasets') / dataset_name) / 'train.txt'), 'wb') as f_out:
                    shutil.copyfileobj(f_in, f_out)
            with gzip.open((((Path(flair.cache_root) / 'datasets') / dataset_name) / 'test.txt.gz'), 'rb') as f_in:
                with open((((Path(flair.cache_root) / 'datasets') / dataset_name) / 'test.txt'), 'wb') as f_out:
                    shutil.copyfileobj(f_in, f_out)
        super(CONLL_2000, self).__init__(data_folder, columns,
                                         tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class GERMEVAL(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=True):
        ...
        if (not data_folder.exists()):
            log.warning(('-' * 100))
            log.warning(''.join(
                ['ACHTUNG: GermEval-14 dataset not found at "', '{}'.format(data_folder), '".']))
            log.warning(
                'Instructions for obtaining the data can be found here: https://sites.google.com/site/germeval2014ner/home/"')
            log.warning(('-' * 100))
        super(GERMEVAL, self).__init__(data_folder, columns,
                                       tag_to_bioes=tag_to_bioes, comment_symbol='#', in_memory=in_memory)


class IMDB(ClassificationCorpus):

    def __init__(self, base_path=None, in_memory=False):
        dataset_name = self.__class__.__name__.lower()
        if (not base_path):
            base_path = (Path(flair.cache_root) / 'datasets')
        data_folder = (base_path / dataset_name)
        ...
        if (not data_file.is_file()):
            ...
            import tarfile
            with tarfile.open((((Path(flair.cache_root) / 'datasets') / dataset_name) / 'aclImdb_v1.tar.gz'), 'r:gz') as f_in:
                datasets = ['train', 'test']
                labels = ['pos', 'neg']
                for label in labels:
        ...
        super(IMDB, self).__init__(data_folder,
                                   use_tokenizer=False, in_memory=in_memory)
...
class NEWSGROUPS(ClassificationCorpus):
    ...
    def __init__(self, base_path=None, in_memory=False):
        ...
        if (not data_file.is_file()):
            cached_path(twenty_newsgroups_path,
                        ((Path('datasets') / dataset_name) / 'original'))
            import tarfile
            with tarfile.open(((((Path(flair.cache_root) / 'datasets') / dataset_name) / 'original') / '20news-bydate.tar.gz'), 'r:gz') as f_in:
                datasets = ['20news-bydate-test', '20news-bydate-train']
                labels = ['alt.atheism', 'comp.graphics', 'comp.os.ms-windows.misc', 'comp.sys.ibm.pc.hardware', 'comp.sys.mac.hardware', 'comp.windows.x', 'misc.forsale', 'rec.autos', 'rec.motorcycles', 'rec.sport.baseball',
                ...
                for label in labels:
                    for dataset in datasets:
                        f_in.extractall((data_path / 'original'), members=[m for m in f_in.getmembers(
                        ) if (''.join(['{}'.format(dataset), '/', '{}'.format(label)]) in m.name)])
                        with open(''.join(['{}'.format(data_path), '/', '{}'.format(dataset), '.txt']), 'at') as f_p:
                            current_path = (
                                ((data_path / 'original') / dataset) / label)
                            for file_name in current_path.iterdir():
                                if file_name.is_file():
                                    f_p.write(((''.join(['__label__', '{}'.format(label), ' ']) + file_name.open(
                                        'rt', encoding='latin1').read().replace('\n', ' <n> ')) + '\n'))
        super(NEWSGROUPS, self).__init__(data_folder,
                                         use_tokenizer=False, in_memory=in_memory)
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=True):
        ...
        if (not data_file.is_file()):
            cached_path(''.join(['{}'.format(
                ner_basque_path), '/eiec_v1.0.tgz']), (Path('datasets') / dataset_name))
            import tarfile
            import shutil
            with tarfile.open((((Path(flair.cache_root) / 'datasets') / dataset_name) / 'eiec_v1.0.tgz'), 'r:gz') as f_in:
        ...
        super(NER_BASQUE, self).__init__(data_folder, columns,
                                         tag_to_bioes=tag_to_bioes, in_memory=in_memory)


class TREC_50(ClassificationCorpus):

    def __init__(self, base_path=None, in_memory=True):
        dataset_name = self.__class__.__name__.lower()
        if (not base_path):
            base_path = (Path(flair.cache_root) / 'datasets')
        data_folder = (base_path / dataset_name)
        ...
        for original_filename in original_filenames:
            cached_path(''.join(['{}'.format(trec_path), '{}'.format(
                original_filename)]), ((Path('datasets') / dataset_name) / 'original'))
        ...
        super(TREC_50, self).__init__(data_folder,
                                      use_tokenizer=False, in_memory=in_memory)
                ...
                with open(((data_folder / 'original') / original_filename), 'rt', encoding='latin1') as open_fp:
                    with open((data_folder / new_filename), 'wt', encoding='utf-8') as write_fp:
                        for line in open_fp:
                            ...
                            write_fp.write(
                                ''.join(['{}'.format(new_label), ' ', '{}'.format(question), '\n']))
        super(TREC_6, self).__init__(data_folder,
                                     use_tokenizer=False, in_memory=in_memory)
...
class UD_ENGLISH(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            web_path), '/en_ewt-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            web_path), '/en_ewt-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            web_path), '/en_ewt-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_GERMAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/de_gsd-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/de_gsd-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/de_gsd-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_GERMAN_HDT(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=False):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/de_hdt-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/de_hdt-ud-test.conllu']), (Path('datasets') / dataset_name))
        train_filenames = ['de_hdt-ud-train-a-1.conllu', 'de_hdt-ud-train-a-2.conllu',
                           'de_hdt-ud-train-b-1.conllu', 'de_hdt-ud-train-b-2.conllu']
        for train_file in train_filenames:
            cached_path(''.join(['{}'.format(ud_path), '/', '{}'.format(train_file)]),
                        ((Path('datasets') / dataset_name) / 'original'))
        data_path = ((Path(flair.cache_root) / 'datasets') / dataset_name)
...
class UD_DUTCH(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/nl_alpino-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/nl_alpino-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/nl_alpino-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_FRENCH(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/fr_gsd-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/fr_gsd-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/fr_gsd-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_ITALIAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/it_isdt-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/it_isdt-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/it_isdt-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_SPANISH(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/es_gsd-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/es_gsd-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/es_gsd-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_PORTUGUESE(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/pt_bosque-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/pt_bosque-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/pt_bosque-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_ROMANIAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/ro_rrt-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ro_rrt-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ro_rrt-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_CATALAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/ca_ancora-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ca_ancora-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ca_ancora-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_POLISH(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/pl_lfg-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/pl_lfg-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/pl_lfg-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_CZECH(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=False):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/cs_pdt-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/cs_pdt-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(ud_path), '/cs_pdt-ud-train-c.conllu']),
                    ((Path('datasets') / dataset_name) / 'original'))
        cached_path(''.join(['{}'.format(ud_path), '/cs_pdt-ud-train-l.conllu']),
                    ((Path('datasets') / dataset_name) / 'original'))
        cached_path(''.join(['{}'.format(ud_path), '/cs_pdt-ud-train-m.conllu']),
                    ((Path('datasets') / dataset_name) / 'original'))
        cached_path(''.join(['{}'.format(ud_path), '/cs_pdt-ud-train-v.conllu']),
                    ((Path('datasets') / dataset_name) / 'original'))
        data_path = ((Path(flair.cache_root) / 'datasets') / dataset_name)
        train_filenames = ['cs_pdt-ud-train-c.conllu', 'cs_pdt-ud-train-l.conllu',
                           'cs_pdt-ud-train-m.conllu', 'cs_pdt-ud-train-v.conllu']
        new_train_file = (data_path / 'cs_pdt-ud-train-all.conllu')
        if (not new_train_file.is_file()):
            with open(new_train_file, 'wt') as f_out:
                for train_filename in train_filenames:
                    with open(((data_path / 'original') / train_filename), 'rt') as f_in:
...
class UD_SLOVAK(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/sk_snk-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/sk_snk-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/sk_snk-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_SWEDISH(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/sv_talbanken-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/sv_talbanken-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/sv_talbanken-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_DANISH(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/da_ddt-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/da_ddt-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/da_ddt-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_NORWEGIAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/no_bokmaal-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/no_bokmaal-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/no_bokmaal-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_FINNISH(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/fi_tdt-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/fi_tdt-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/fi_tdt-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_SLOVENIAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/sl_ssj-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/sl_ssj-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/sl_ssj-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_CROATIAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/hr_set-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/hr_set-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/hr_set-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_SERBIAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/sr_set-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/sr_set-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/sr_set-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_BULGARIAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/bg_btb-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/bg_btb-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/bg_btb-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_ARABIC(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/ar_padt-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ar_padt-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ar_padt-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_HEBREW(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/he_htb-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/he_htb-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/he_htb-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_TURKISH(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/tr_imst-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/tr_imst-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/tr_imst-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_PERSIAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/fa_seraji-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/fa_seraji-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/fa_seraji-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_RUSSIAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/ru_syntagrus-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ru_syntagrus-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ru_syntagrus-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_HINDI(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/hi_hdtb-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/hi_hdtb-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/hi_hdtb-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_INDONESIAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/id_gsd-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/id_gsd-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/id_gsd-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_JAPANESE(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/ja_gsd-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ja_gsd-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ja_gsd-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_CHINESE(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/zh_gsd-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/zh_gsd-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/zh_gsd-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_KOREAN(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/ko_kaist-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ko_kaist-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/ko_kaist-ud-train.conllu']), (Path('datasets') / dataset_name))
...
class UD_BASQUE(UniversalDependenciesCorpus):
    ...
    def __init__(self, base_path=None, in_memory=True):
        ...
        cached_path(''.join(['{}'.format(
            ud_path), '/eu_bdt-ud-dev.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/eu_bdt-ud-test.conllu']), (Path('datasets') / dataset_name))
        cached_path(''.join(['{}'.format(
            ud_path), '/eu_bdt-ud-train.conllu']), (Path('datasets') / dataset_name))
...
def _download_wassa_if_not_there(emotion, data_folder, dataset_name):
    for split in ['train', 'dev', 'test']:
        data_file = (
            data_folder / ''.join(['{}'.format(emotion), '-', '{}'.format(split), '.txt']))
        if (not data_file.is_file()):
            if (split == 'train'):
...
class WASSA_ANGER(ClassificationCorpus):
    ...
    def __init__(self, base_path=None, in_memory=False):
        ...
        super(WASSA_ANGER, self).__init__(data_folder,
                                          use_tokenizer=False, in_memory=in_memory)
...
class WASSA_FEAR(ClassificationCorpus):
    ...
    def __init__(self, base_path=None, in_memory=False):
        ...
        super(WASSA_FEAR, self).__init__(data_folder,
                                         use_tokenizer=False, in_memory=in_memory)
...
class WASSA_JOY(ClassificationCorpus):
    ...
    def __init__(self, base_path=None, in_memory=False):
        ...
        super(WASSA_JOY, self).__init__(data_folder,
                                        use_tokenizer=False, in_memory=in_memory)
...
class WASSA_SADNESS(ClassificationCorpus):
    ...
    def __init__(self, base_path=None, in_memory=False):
        ...
        super(WASSA_SADNESS, self).__init__(data_folder,
                                            use_tokenizer=False, in_memory=in_memory)


def _download_wikiner(language_code, dataset_name):
    wikiner_path = 'https://raw.githubusercontent.com/dice-group/FOX/master/input/Wikiner/'
    lc = language_code
    data_file = (((Path(flair.cache_root) / 'datasets') / dataset_name) /
                 ''.join(['aij-wikiner-', '{}'.format(lc), '-wp3.train']))
    if (not data_file.is_file()):
        cached_path(''.join(['{}'.format(wikiner_path), 'aij-wikiner-',
                             '{}'.format(lc), '-wp3.bz2']), (Path('datasets') / dataset_name))
        import bz2
        import shutil
        bz_file = bz2.BZ2File((((Path(flair.cache_root) / 'datasets') / dataset_name) /
                               ''.join(['aij-wikiner-', '{}'.format(lc), '-wp3.bz2'])), 'rb')
        with bz_file as f, open((((Path(flair.cache_root) / 'datasets') / dataset_name) / ''.join(['aij-wikiner-', '{}'.format(lc), '-wp3.train'])), 'w') as out:
            for line in f:
...
class WIKINER_ENGLISH(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=False):
        columns = {
            0: 'text',
            1: 'pos',
            2: 'ner',
        ...
        super(WIKINER_ENGLISH, self).__init__(data_folder, columns,
                                              tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class WIKINER_GERMAN(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=False):
        ...
        super(WIKINER_GERMAN, self).__init__(data_folder, columns,
                                             tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class WIKINER_DUTCH(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=False):
        ...
        super(WIKINER_DUTCH, self).__init__(data_folder, columns,
                                            tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class WIKINER_FRENCH(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=False):
        ...
        super(WIKINER_FRENCH, self).__init__(data_folder, columns,
                                             tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class WIKINER_ITALIAN(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=False):
        ...
        super(WIKINER_ITALIAN, self).__init__(data_folder, columns,
                                              tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class WIKINER_SPANISH(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=False):
        ...
        super(WIKINER_SPANISH, self).__init__(data_folder, columns,
                                              tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class WIKINER_PORTUGUESE(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=False):
        ...
        super(WIKINER_PORTUGUESE, self).__init__(data_folder,
                                                 columns, tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class WIKINER_POLISH(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=False):
        ...
        super(WIKINER_POLISH, self).__init__(data_folder, columns,
                                             tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class WIKINER_RUSSIAN(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=False):
        ...
        super(WIKINER_RUSSIAN, self).__init__(data_folder, columns,
                                              tag_to_bioes=tag_to_bioes, in_memory=in_memory)
...
class WNUT_17(ColumnCorpus):
    ...
    def __init__(self, base_path=None, tag_to_bioes='ner', in_memory=True):
        ...
        cached_path(''.join(['{}'.format(wnut_path), 'emerging.test.annotated']), (Path(
            'datasets') / dataset_name))
        super(WNUT_17, self).__init__(data_folder, columns,
                                      tag_to_bioes=tag_to_bioes, in_memory=in_memory)


class DataLoader(torch.utils.data.dataloader.DataLoader):

    def __init__(self, dataset, batch_size=1, shuffle=False, sampler=None, batch_sampler=None, num_workers=8, drop_last=False, timeout=0, worker_init_fn=None):
        ...
        super(DataLoader, self).__init__(dataset, batch_size=batch_size, shuffle=shuffle, sampler=sampler, batch_sampler=batch_sampler,
                                         num_workers=num_workers, collate_fn=list, drop_last=drop_last, timeout=timeout, worker_init_fn=worker_init_fn)
