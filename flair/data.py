...
class Sentence(DataPoint):
    ...
    def __init__(self, text: str = None, use_tokenizer: bool = False, labels: Union[(List[Label], List[str])] = None, language_code: str = None):
        ...
        for token in self.tokens:
            ...
            for tag_type in token.tags.keys():
                if ((main_tag is not None) and (main_tag != tag_type)):
                    continue
                if ((token.get_tag(tag_type).value == '') or (token.get_tag(tag_type).value == 'O')):
                    continue
    ...
    def __str__(self) -> str:
        if self.labels:
            return ''.join(['Sentence: "', '{}'.format(self.to_tokenized_string()), '" - ', '{}'.format(len(self)), ' Tokens - Labels: ', '{}'.format(self.labels), ' '])
        else:
            return ''.join(['Sentence: "', '{}'.format(self.to_tokenized_string()), '" - ', '{}'.format(len(self)), ' Tokens'])
...
class Corpus():
    ...
    def __init__(self, train: FlairDataset, dev: FlairDataset, test: FlairDataset, name: str = 'corpus'):
        self._train = train
        self._dev = dev
        self._test = test
        self.name = name
    ...
    def _downsample_to_proportion(self, dataset: Dataset, proportion: float):
        sampled_size = round((len(dataset) * proportion))
        splits = random_split(
            dataset, [(len(dataset) - sampled_size), sampled_size])
        return splits[1]
...
class MultiCorpus(Corpus):
    ...
    def __init__(self, corpora: List[Corpus], name: str = 'multicorpus'):
        ...
        super(MultiCorpus, self).__init__(ConcatDataset([corpus.train for corpus in self.corpora]), ConcatDataset(
            [corpus.dev for corpus in self.corpora]), ConcatDataset([corpus.test for corpus in self.corpora]), name=name)
