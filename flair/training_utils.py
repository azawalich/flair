...
class Metric(object):
    ...
    def f_score(self, class_name=None):
        if ((self.precision(class_name) + self.recall(class_name)) > 0):
            return round(((2 * (self.precision(class_name) * self.recall(class_name))) / (self.precision(class_name) + self.recall(class_name))), 4)
        return 0.0

    def accuracy(self, class_name=None):
        if (((self.get_tp(class_name) + self.get_fp(class_name)) + self.get_fn(class_name)) > 0):
            return round((self.get_tp(class_name) / ((self.get_tp(class_name) + self.get_fp(class_name)) + self.get_fn(class_name))), 4)
        return 0.0
    ...
    def macro_avg_f_score(self):
        class_f_scores = [self.f_score(class_name)
    ...
    def macro_avg_accuracy(self):
        class_accuracy = [self.accuracy(class_name)
                          for class_name in self.get_classes()]
    ...
    def get_classes(self) -> List:
        ...
        all_classes = [class_name for class_name in all_classes if (
            class_name is not None)]
        all_classes.sort()
        return all_classes

    def to_tsv(self):
        return '{}\t{}\t{}\t{}'.format(self.precision(), self.recall(), self.accuracy(), self.micro_avg_f_score())
    ...
    def __str__(self):
        ...
        all_lines = ['{0:<10}\ttp: {1} - fp: {2} - fn: {3} - tn: {4} - precision: {5:.4f} - recall: {6:.4f} - accuracy: {7:.4f} - f1-score: {8:.4f}'.format((self.name if (class_name is None) else class_name), self.get_tp(
            class_name), self.get_fp(class_name), self.get_fn(class_name), self.get_tn(class_name), self.precision(class_name), self.recall(class_name), self.accuracy(class_name), self.f_score(class_name)) for class_name in all_classes]
        return '\n'.join(all_lines)
...
class MetricRegression(object):
    ...
    def to_tsv(self):
        return '{}\t{}\t{}\t{}'.format(self.mean_squared_error(), self.mean_absolute_error(), self.pearsonr(), self.spearmanr())
...
class WeightExtractor(object):
    ...
    def extract_weights(self, state_dict, iteration):
        for key in state_dict.keys():
            vec = state_dict[key]
            weights_to_watch = min(self.number_of_weights, reduce(
                (lambda x, y: (x * y)), list(vec.size())))
